...
describe("GET /todos/:id", () => {
    ...
    it("should not return todo doc created by other user", done => {
        ...
        request(app).get(`/todos/${(new ObjectId()).toHexString()}`).set('x-auth', users[0].tokens[0].token).expect(404).end(done);
    });
    it("should return 404 if todo not found", done => {
        var hexId = new ObjectId().toHexString();
        request(app).get(`/todos/${(new ObjectId()).toHexString()}`).set('x-auth', users[1].tokens[0].token).expect(404).end(done);
    });
    it("should return 400 for non-object ids", done => {
        request(app).get("/todos/123abc").set('x-auth', users[0].tokens[0].token).expect(400).end(done);
    });
});
...
describe("DELETE /todos/:id", () => {
    ...
    it("should return 404 if another user deletes another user's todo", done => {
    ...
    it("should return 404 if todo not found", done => {
        var hexId = new ObjectId().toHexString();
        request(app).delete(`/todos/${(new ObjectId()).toHexString()}`).set('x-auth', users[1].tokens[0].token).expect(404).end(done);
    });

    it("should return 400 if object id is invalid", done => {
        request(app).delete("/todos/123abc").set('x-auth', users[1].tokens[0].token).expect(400).end(done);
    });
});
...
describe("GET /users/me", () => {
    it("should not return user if authenticated", done => {
